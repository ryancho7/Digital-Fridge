...
export default function AddItem({ fridgeId, setFoodItems }) {
    ...
    const handleAddItem = async (e) => {
        e.preventDefault();
        try {
            const res = await fetch(`/api/foodItems/${fridgeId}`, {  // Modified to use fridgeId
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                },
                body: JSON.stringify({ name: itemName, category: itemCategory, quantity, unit, expirationDate })
            });
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md mt-6">
            ...
            <form onSubmit={handleAddItem} className="flex flex-col gap-4">
                ...
                <div>
                    <label htmlFor="expirationDate" className="text-sm font-semibold text-gray-700">Expiration Date</label>
                    <input
                        id="expirationDate"
                        type="date"
